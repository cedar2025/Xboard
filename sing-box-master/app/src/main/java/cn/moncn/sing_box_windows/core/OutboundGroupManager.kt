...
object OutboundGroupManager {
    ...
    suspend fun urlTest(outboundTag: String): Result<Unit> {
        ...
        updateTestingState(outboundTag, true)

        val encodedProxy = encodePathSegment(outboundTag)
        android.util.Log.e("OutboundGroupManager", "Starting speed test for: $outboundTag (encoded: $encodedProxy)")
        val result = runCatching {
            android.util.Log.e("OutboundGroupManager", "Calling ClashApiClient.getJsonObject...")
            val json = ClashApiClient.getJsonObject(
                "/proxies/$encodedProxy/delay",
                mapOf("url" to TEST_URL, "timeout" to TEST_TIMEOUT_MS.toString()),
                timeoutMs = 10000 // Give plenty of time for response
            )
            android.util.Log.e("OutboundGroupManager", "Got response: $json")
            json
        }.map { response ->
            ...
            if (delay != null) {
                ...
                android.util.Log.e("OutboundGroupManager", "Invalid delay from response: $response")
                updateTestingState(outboundTag, false) // Stop spinner, don't fake it
            }
        }.onFailure { e ->
            updateTestingState(outboundTag, false)
            android.util.Log.e("OutboundGroupManager", "urlTest failed for $outboundTag", e)
        }
        return result
    }

    private fun updateTestingState(tag: String, isTesting: Boolean) {
        mainHandler.post {
            if (isTesting) {
                val baseline = lastTestSnapshot[tag]
                testingTags = testingTags + (tag to TestingState(baseline, System.currentTimeMillis()))
            } else {
