...
```json
{
    ...
    "type": "payment",
    "version": "1.3.0",
    "description": "Stripe payment plugin for Xboard. Supports Card, WeChat Pay, Alipay, and Cryptocurrency.",
    "author": "Xboard Team"
}
```
...
# 检查插件目录
ls -la plugins/
ls -la plugins/Stripe/

# 确保文件可读
cat plugins/Stripe/config.json
```
...
2. **检查文件内容**
   ```bash
   # 验证 JSON 格式
   cat plugins/Stripe/config.json | python -m json.tool
   ```
...
插件管理器通过以下逻辑查找插件：
- 插件代码：`stripe`
- 转换为文件夹名：`Str::studly('stripe')` = `Stripe`
- 完整路径：`base_path('plugins') . '/Stripe'`
- 配置文件路径：`plugins/Stripe/config.json`
