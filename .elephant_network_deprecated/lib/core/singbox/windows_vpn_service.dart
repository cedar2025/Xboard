...
class WindowsVpnService implements VpnManager {
  ...
  Future<void> start(String config) async {
    try {
      ...
      final binFile = File('${singboxDir.path}/sing-box.exe');
      ...
      for (final filePath in filesToDelete) {
        ...
        if (await f.exists()) {
          try {
            ...
            debugPrint('WindowsVpnService: failed to delete $filePath on start: $e');
          }
        }
      }
  ...
  Future<void> stop() async {
    try {
      ...
      if (_singboxProcess != null) {
        ...
        await Process.run('taskkill', ['/F', '/IM', 'sing-box-windows-amd64.exe']);
      }
  ...
  Future<void> selectOutbound(String groupTag, String outboundTag) async {
    if (!_isConnected || _lastSanitizedConfig == null || _singboxDirPath == null || _singboxBinPath == null) {
      debugPrint('WindowsVpnService selectOutbound: not connected or no stored config, skipping');
      return;
    }

    debugPrint('WindowsVpnService selectOutbound: switching $groupTag -> $outboundTag via restart');
    ...
    try {
      ...
      if (config.containsKey('outbounds') && config['outbounds'] is List) {
        ...
        for (var outbound in outbounds) {
          if (outbound is Map && outbound['type'] == 'selector' && outbound['tag'] == groupTag) {
            ...
            debugPrint('WindowsVpnService: set selector "$groupTag" default to "$outboundTag"');
            break;
          }
        }
      }
      ...
      debugPrint('WindowsVpnService selectOutbound: restart complete, now using "$outboundTag"');
    } catch (e) {
